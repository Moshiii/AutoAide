...
test("control plane web server exposes logs and config update endpoints", async () => {
  await withTempHome(async (tempHome) => {
    ...
    try {
      ...
      assert.match(homeHtml, /data-nav="bots"/);
      assert.match(homeHtml, /data-nav="runs"/);
      assert.match(homeHtml, /data-nav="users"/);
      assert.match(homeHtml, /data-nav="settings"/);
      assert.doesNotMatch(homeHtml, /class="topbar"/);
      assert.doesNotMatch(homeHtml, /id="inspector-root"/);
      assert.match(homeHtml, /__openSetupStep/);
      assert.match(homeHtml, /Global dashboard for bots, users, conversations, runs, workspace usage, and credits/);
      assert.match(homeHtml, /Bot status/);
      assert.match(homeHtml, /Total credits consumed/);
      assert.match(homeHtml, /Total credits granted/);
      ...
      assert.match(homeHtml, /Run quick file demo/);
      ...
      assert.match(homeHtml, /User list/);
      assert.match(homeHtml, /Only users seen by the current bot are shown here/);
      assert.match(homeHtml, /operations-users/);
      assert.match(homeHtml, /No users yet\. Invite a user/);
      ...
      assert.match(homeHtml, /Start a local chat or connect Feishu\/Telegram/);
      assert.match(homeHtml, /External sharing restricted/);
      assert.match(homeHtml, /Local and trusted-user testing can continue/);
